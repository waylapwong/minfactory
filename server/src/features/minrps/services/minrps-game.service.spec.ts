...
describe('MinRpsGameService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        MinRpsGameService,
        {
          ...
          useValue: MINRPS_GAME_REPOSITORY_MOCK,
        },
        {
          provide: MinRpsMatchRepository,
          useValue: MINRPS_MATCH_REPOSITORY_MOCK,
        },
      ],
    }).compile();
  ...
  describe('getAllGames', () => {
    it('should return all games', async () => {
      ...
      expect(MINRPS_GAME_REPOSITORY_MOCK.findAll).toHaveBeenCalled();
    });
    ...
    it('should map observer and player counts from match repository state', async () => {
      ...
      expect(MINRPS_MATCH_REPOSITORY_MOCK.findOne).toHaveBeenCalledWith('1');
    });
  ...
  describe('getGame', () => {
    ...
    it('should map observer and player counts from match repository in getGame', async () => {
      ...
      expect(MINRPS_MATCH_REPOSITORY_MOCK.findOne).toHaveBeenCalledWith('test-id');
    });
  });
});
