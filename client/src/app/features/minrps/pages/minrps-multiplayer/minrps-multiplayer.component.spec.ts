...
describe('MinRpsMultiplayerComponent', () => {
  ...
  describe('ngOnInit()', () => {
    ...
    it('should subscribe to all events', () => {
      expect(mockMultiplayerService.onEvent).toHaveBeenCalledWith(
        MinRpsGameEvent.Connected,
        jasmine.any(Function),
      );
      expect(mockMultiplayerService.onEvent).toHaveBeenCalledWith(
        MinRpsGameEvent.Joined,
        jasmine.any(Function),
      );
      expect(mockMultiplayerService.onEvent).toHaveBeenCalledWith(
        MinRpsGameEvent.Left,
        jasmine.any(Function),
      );
      expect(mockMultiplayerService.onEvent).toHaveBeenCalledWith(
      ...
      expect(mockMultiplayerService.onEvent).toHaveBeenCalledWith(
        MinRpsGameEvent.MoveSelected,
        jasmine.any(Function),
      );
      expect(mockMultiplayerService.onEvent).toHaveBeenCalledWith(
        MinRpsGameEvent.Played,
        jasmine.any(Function),
      );
      expect(mockMultiplayerService.onEvent).toHaveBeenCalledWith(
  ...
  describe('ngOnDestroy()', () => {
    it('should unsubscribe from all events', () => {
      ...
      expect(mockMultiplayerService.offEvent).toHaveBeenCalledWith(
        MinRpsGameEvent.Connected,
        jasmine.any(Function),
      );
      expect(mockMultiplayerService.offEvent).toHaveBeenCalledWith(
  ...
  describe('submitText computed signal', () => {
    ...
    it('should return "play paper!" when paper is selected', () => {
      component.game.update((g) => ({
        ...g,
        playerSelectedMove: MinRpsMove.Paper,
        playerHasSelectedMove: false,
      }));
    ...
    it('should return "play scissors!" when scissors is selected', () => {
      component.game.update((g) => ({
        ...g,
        playerSelectedMove: MinRpsMove.Scissors,
        playerHasSelectedMove: false,
      }));
  ...
  describe('playGame()', () => {
    it('should send play event when is player', () => {
      ...
      component.playGame();
      expect(mockMultiplayerService.sendPlayEvent).toHaveBeenCalledWith(
  ...
  describe('selectMove()', () => {
    it('should update game state and send event when is player', () => {
      ...
      expect(mockMultiplayerService.sendSelectMoveEvent).toHaveBeenCalledWith(
        jasmine.objectContaining({
          gameId: 'game-id',
          playerId: 'p1',
